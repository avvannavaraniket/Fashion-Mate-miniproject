...
import { SendIcon, HangerIcon, RefreshCwIcon, SparklesIcon, CameraIcon, XIcon } from './components/Icons';
...
const App: React.FC = () => {
  ...
  const validate = useCallback((field: 'occasion' | 'preferences' | 'gender', value: string) => {
    ...
    if (field === 'occasion') {
      if (!trimmed && !image) {
        // If image exists, text is optional/less strict
        message = 'Please describe the occasion or upload a photo.';
      } else if (trimmed && trimmed.length < OCCASION_MIN_LENGTH && !image) {
  ...
  const handleImageUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        // Extract base64 data for API (remove "data:image/jpeg;base64," prefix)
        const base64Data = resultStr.split(',')[1];
        setImage({
          data: base64Data,
          mimeType: file.type
        });
        // Clear occasion error if it was "required"
        setValidationErrors(prev => ({...prev, occasion: ''}));
      };
  ...
  const removeImage = () => {
    ...
    validate('occasion', occasion); // Re-validate
  };
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    if ((!occasion.trim() && !image) || !gender) return;
  ...
  }, [occasion, gender, preferences, image, validate]);
  ...
  const isFormValid = !validationErrors.occasion && !validationErrors.gender && !validationErrors.preferences && (occasion.trim().length >= OCCASION_MIN_LENGTH || image) && gender;
          ...
          <div className="max-w-3xl mx-auto text-center animate-fade-in-up">
            ...
            <form onSubmit={handleSubmit} className="relative z-10 max-w-xl mx-auto text-left">
              ...
              <div className="bg-white/70 backdrop-blur-xl rounded-[2.5rem] p-2 shadow-[0_20px_50px_-12px_rgba(0,0,0,0.1)] border border-white/50 transition-all focus-within:shadow-[0_25px_60px_-12px_rgba(0,0,0,0.15)] focus-within:bg-white/90">
                <div className="p-6 relative">
                    ...
                    {imagePreview && (
                        <div className="mb-4 relative inline-block group">
                            <div className="relative overflow-hidden rounded-2xl border border-stone-100 shadow-sm w-32 h-32 md:w-40 md:h-40">
                    ...
                    <textarea
                        ...
                        placeholder={image ? "Add any specific details about the item..." : "e.g. A gallery opening in Soho, minimal but chic..."}
                    ...
                    {/* Image Upload Button within Text Area */}
                    <div className="absolute bottom-6 right-6">
                        ...
                        <label 
                            ...
                            className="cursor-pointer flex items-center gap-2 px-4 py-2 bg-stone-100/50 hover:bg-stone-100 text-stone-500 hover:text-stone-800 rounded-full transition-all text-xs font-bold uppercase tracking-wide border border-stone-200/50"
                            title="Upload a photo of a clothing item or inspiration"
                        >
                ...
                <div className="border-t border-stone-100/50 p-6">
                    <div className="flex justify-between items-center mb-3 px-1">
                        <label htmlFor="preferences" className="text-xs font-bold text-stone-400 uppercase tracking-wider">Style Profile (Optional)</label>
